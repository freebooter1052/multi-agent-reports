...
def fetch_with_retry(url, max_retries=5):
    """Fetches a URL with exponential backoff and a User-Agent header."""
    ...
    for attempt in range(max_retries):
        try:
            ...
            if e.code in (429, 503, 301) or 500 <= e.code < 600:
                print(f"HTTP {e.code} encountered for {url}. Retrying ({attempt + 1}/{max_retries})...")
                time.sleep((2 ** attempt)) # Exponential backoff
            else:
                raise
        except urllib.error.URLError as e:
            print(f"URL Error encountered for {url}: {e.reason}. Retrying ({attempt + 1}/{max_retries})...")
