...
def fetch_url_with_retry(url, max_retries=5):
    """Fetches a URL with exponential backoff for 429, 503, and 301 errors."""
    ...
    for attempt in range(max_retries):
        try:
            ...
            if e.code in [429, 503, 301] and attempt < max_retries - 1:
                sleep_time = 2 ** attempt
                print(f"HTTPError {e.code} for {url}. Retrying in {sleep_time} seconds...")
            ...
            if attempt < max_retries - 1:
                sleep_time = 2 ** attempt
                print(f"URLError {e.reason} for {url}. Retrying in {sleep_time} seconds...")
                time.sleep(sleep_time)
            else:
                raise
...
def download_pdf(pdf_url, dir_path):
    ...
    data = fetch_url_with_retry(pdf_url)
