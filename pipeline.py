...
def urlopen_with_retry(request, max_retries=5):
    """Wraps urllib.request.urlopen with retry logic for 301, 429, 503 errors."""
    if isinstance(request, str):
        request = urllib.request.Request(request)

    # Always include User-Agent for arXiv
        ...
        try:
            ...
            if e.code in (301, 429, 503):
                if attempt == max_retries - 1:
                    raise
                print(f"HTTP {e.code} encountered. Retrying in {delay} seconds...")
                time.sleep(delay)
                delay *= 2
            else:
                raise
            ...
            if attempt == max_retries - 1:
                raise
            print(f"URLError encountered: {e.reason}. Retrying in {delay} seconds...")
            time.sleep(delay)
            delay *= 2
...
def download_pdf(pdf_url, dir_path):
    ...
    with urlopen_with_retry(pdf_url) as response, open(pdf_path, 'wb') as out_file:
        data = response.read()
        out_file.write(data)
    return pdf_path
