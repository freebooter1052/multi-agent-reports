...
def fetch_with_retry(url, max_retries=5, initial_backoff=1.0):
    retries = 0
    ...
    while retries < max_retries:
        ...
        try:
            ...
            if e.code in [429, 503, 301]:
                print(f"HTTP {e.code} received, retrying in {backoff} seconds...")
                time.sleep(backoff)
                backoff *= 2
                retries += 1
            else:
            ...
            print(f"Exception {e} received, retrying in {backoff} seconds...")
            time.sleep(backoff)
            backoff *= 2
            retries += 1
