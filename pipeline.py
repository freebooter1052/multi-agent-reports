    ...
    for attempt in range(max_retries):
        try:
            ...
            if e.code in (429, 503, 301):
                ...
                print(f"HTTPError {e.code} in fetch_latest_papers, retrying in {delay} seconds...")
                time.sleep(delay)
            else:
                raise
        except URLError as e:
            delay = base_delay * (2 ** attempt)
            print(f"URLError {e.reason} in fetch_latest_papers, retrying in {delay} seconds...")
            time.sleep(delay)

    if data is None:
        raise Exception("Failed to fetch latest papers after max retries")
...
def download_pdf(pdf_url, dir_path):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if e.code in (429, 503, 301):
                delay = base_delay * (2 ** attempt)
                print(f"HTTPError {e.code} in download_pdf, retrying in {delay} seconds...")
                time.sleep(delay)
            else:
                raise
        except URLError as e:
            delay = base_delay * (2 ** attempt)
            print(f"URLError {e.reason} in download_pdf, retrying in {delay} seconds...")
            time.sleep(delay)

    if data is None:
        raise Exception(f"Failed to download PDF {pdf_url} after max retries")
