...
def fetch_url_with_retry(url, retries=5, backoff_factor=1):
    req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0'})
    for i in range(retries):
        try:
            return urllib.request.urlopen(req)
        except urllib.error.HTTPError as e:
            if e.code in [429, 503, 301] and i < retries - 1:
            ...
            if i < retries - 1:
                ...
                raise
    raise Exception("Failed to fetch URL after retries")
...
def fetch_latest_papers(max_results=3):
    ...
    query = 'all:"multi-agent" AND all:"communication"'
...
def download_pdf(pdf_url, dir_path):
    ...
    response = fetch_url_with_retry(pdf_url)
