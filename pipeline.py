    ...
    for i in range(max_retries):
        try:
            ...
            if e.code in [429, 503, 301]:
                sleep_time = (2 ** i)
                print(f"HTTP {e.code} encountered. Retrying in {sleep_time} seconds...")
                time.sleep(sleep_time)
            else:
                raise e
        except urllib.error.URLError as e:
            sleep_time = (2 ** i)
            print(f"URL Error encountered: {e}. Retrying in {sleep_time} seconds...")
            time.sleep(sleep_time)
...
Explain it like you are explaining to a higher secondary student and very entry-level developers. Strictly avoid unexplained jargon. Keep the tone technical, objective, and clear. Do not hallucinate details. If a societal benefit is not explicitly mentioned, infer a logical one based strictly on its specialized sector utility.
Abstract: {abstract}
Excerpt: {text[:10000]}
